...
import torch
import os
from models.sony_images import sid_bottleneck_transformer, sid_bottleneck_transformer_relpos

EXPORT_DIR = './models/sony_images/states/tf_export/'
MODEL_STATE_FILE= './models/sony_images/states/sid_bottleneck_transformer_initial_relpos_2b_sym.pt'
...
model = sid_bottleneck_transformer_relpos.Model_2b_Sym()
...
for root, dirs, files in os.walk(EXPORT_DIR):
    for file in files:
        ...
        model_name = None
        if name in ENCODER_DECODER_MAP:
            model_name = ENCODER_DECODER_MAP[name]
        elif IMPORT_BOTTLENECK and name in BOTTLENECK_MAP:
            model_name = BOTTLENECK_MAP[name]
        
        if model_name is not None:            
            if 'weight' in model_name:
                tensor = torch.from_numpy(tensor).permute(3, 2, 0, 1)
            else:
                tensor = torch.from_numpy(tensor)
            
            print(name + ' ' + model_name)
            assert state_dict[model_name].shape == tensor.shape
